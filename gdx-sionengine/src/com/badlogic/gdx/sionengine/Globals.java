package com.badlogic.gdx.sionengine;

public class Globals {
	static final public int state_invalid = SionEngine.getIDGenerator().getID("state_invalid");
	static final public int state_idle = SionEngine.getIDGenerator().getID("state_idle");
	static final public int state_erase = SionEngine.getIDGenerator().getID("state_erase");
	
	static final public int type_invalid = SionEngine.getIDGenerator().getID("type_invalid");
}
